...
const approveDoc = async (doc) => {
    const { collectionId, tokenId } = doc.data ();
    ...
    console.log ('approved', tokenId, doc.ref.firestore.projectId);
}
...
const doHealthUpdate = async () => {
    ...
    try {
        ...
        console.log('ERROR in doHealthUpdate=',e.message);
    }
}
const doCheckDocs = async () => {
    for (const doc of currentDocs) {
        try {
            ...
            if(process.env.DEBUG){
                console.log ('checking', doc.id, ' nftId=',doc.data().id);
            }

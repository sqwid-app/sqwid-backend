...
const marketplaceApolloClient = new ApolloClient({
    link: createHttpLink({
        uri: getNetworkConfig().graphql_api_marketplace,
        fetch
    }),
...
console.log("marketplaceAxiosClient====",marketplaceAxiosClient);

const explorerApolloClient = new ApolloClient({
    link: createHttpLink({
        uri: getNetworkConfig().graphql_api_explorer,
        fetch
    }),
    cache: new InMemoryCache(),
});
...
const bidsByBidder = (bidderAddress, offset, limit) => {
    ...
    return { query, entity: null, limit: 1, client: marketplaceApolloClient, mapper };
}
...
const doQuery = async ({query, entity, limit, client, mapper}) => {
    ...
    do {
        ...
        if (!entity) return mapper ? mapper(res.data) : res.data;

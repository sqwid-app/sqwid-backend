...
const { doQuery, transfersQuery, hasAvailablePositionsQuery, nftBalanceQuery, salesQuery, marketplaceHeightQuery, explorerHeightQuery, getMarketplaceHeight, doQuery2 } = require('./gql');
...
const sync = async (_network, fire) => {
    ...
    const marketplaceBlockHeight = await doQuery2(marketplaceHeightQuery (_network));
    // console.log("anunaaa===",await getMarketplaceHeight(_network))
    console.log("anunaaa===",marketplaceBlockHeight)

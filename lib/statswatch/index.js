...
const sync = async (network, fire) => {
    const explorerBlockHeight = await doQuery(explorerHeightQuery(network));
    await onNftContractEvent(explorerBlockHeight, network, fire);

    const marketplaceBlockHeight = await doQuery(marketplaceHeightQuery (network));
    await onMarketItemSold (marketplaceBlockHeight, network, fire);
}

const createSubscriptionClient = (network, fire) => {
    ...
    channel.bind(config [network].pusherEventExplorer, async (data) => {
        ...
        if (data.updatedContracts.includes (config [network].marketplaceContractAddress) ||
            ...
            await onNftContractEvent (data.blockHeight, network, fire);
        }
    });
    ...
    firebase.collection(collectionName).onSnapshot((querySnapshot) => {
        querySnapshot.forEach(async (doc) => {
            if(doc.data().events && doc.data().events.includes(MarketItemSoldEvent)){
                await onMarketItemSold (doc.id, network);
            }
        });
    }, (error) => {
...
const init = async (network, resetStats, resetClaims,fire) => {
    ...
    if (resetStats) await doResetStats (network, fire);
    if (resetClaims) await doResetClaims (network, fire);
    ...
    await sync(network, fire);

    createSubscriptionClient (network, fire);
    if (process.env.DEBUG) {

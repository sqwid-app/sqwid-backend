...
const createSubscriptionClient = (_network, fire) => {
    let selectedNetwork
    try {
        if(!selectedNetwork){
          reefState.setSelectedNetwork(network.AVAILABLE_NETWORKS[_network])
    ...
    // console.log('Start Pusher', _network);
    // const pusher = new Pusher(process.env.PUSHER_KEY, {
    //     cluster: process.env.PUSHER_CLUSTER || "eu",
    // });
    // const channel = pusher.subscribe(process.env.PUSHER_CHANNEL);

    // channel.bind(config [_network].pusherEventExplorer, async (data) => {
    //     if (process.env.DEBUG) {
    //         console.log('event from pusher=',process.env.PUSHER_CHANNEL, ' data=',data);
    //     }
    //     if (data.updatedContracts.includes (config [_network].marketplaceContractAddress) ||
    //         data.updatedContracts.includes (config [_network].nftContractAddress)
    //     ) {
    //         await onNftContractEvent (data.blockHeight, _network, fire);
    //     }
    // });

    // channel.bind(config [_network].pusherEventMarketplace, async (data) => {
    //     if (data.events.includes(MarketItemSoldEvent)) {
    //         await onMarketItemSold (data.blockHeight, _network);
    //     }
    // });
    ...
    firebase.collection(collectionName).onSnapshot((querySnapshot) => {
        querySnapshot.forEach(async (doc) => {
            if(doc.data().events && doc.data().events.includes(MarketItemSoldEvent)){
                await onMarketItemSold (doc.id, _network);
            }
        });
    }, (error) => {
